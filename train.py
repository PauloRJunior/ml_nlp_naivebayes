...
tfidf_transform = TfidfVectorizer().fit_transform(base_treino['text']) #treinamento
...
tfidf_transformer = TfidfTransformer().fit_transform(message_transform)
message_tfidf = tfidf_transformer.transform(message_transform)
print(message_tfidf.shape)

#PARAMETROS DE TESTES
x_train , x_val , y_train , y_val = train_test_split(
    message_tfidf , base_inicial['Category'], test_size = 0.2, random_state = 0)


print(f'Base para treino: {x_train.shape}')
print(f'Base para teste: {y_train.shape}')
...
#TESTE
print('predicted:' , modelo.predict(bow4)[0])
print('expected:' , base_inicial['Category'][3])

#MODELO DE TESTE
label_predictions = modelo.predict(x_val)
print(label_predictions)
print(metrics.classification_report(y_val, label_predictions))
print(metrics.confusion_matrix(y_val, label_predictions))
print('Accurarcy of the model: {0:0.3f}'.format(metrics.accuracy_score(y_val,label_predictions)))
